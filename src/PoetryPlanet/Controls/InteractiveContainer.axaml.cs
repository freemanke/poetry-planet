...
public partial class InteractiveContainer : UserControl
{
    public InteractiveContainer()
    {
        ...
        Loaded += (_, _) =>
        {
            if (TopLevel.GetTopLevel(this) is { } toplevel)
            {
                ...
                {
                    var mb = MobileNavigation.GetMobileStackInstance();
                    if (mb.Pages.Count == 0) return;
    ...
    public static readonly StyledProperty<Control> DialogContentProperty =
        AvaloniaProperty.Register<InteractiveContainer, Control>(nameof(InteractiveContainer),
